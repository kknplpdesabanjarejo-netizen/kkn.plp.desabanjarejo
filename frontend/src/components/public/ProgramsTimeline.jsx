    ...
    <section id="programs" className="py-24 bg-white" data-testid="programs-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {loading ? (
          ...
          <EmptyState title="No Programs Yet" message="Program areas will appear here after the administrator adds them." testId="programs-empty" />
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {sorted.map((p, i) => (
              <motion.div
    ...
    <section id="timeline" className="py-24 bg-slate-950 relative overflow-hidden" data-testid="timeline-section">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative">
        <div className="max-w-2xl mb-14">
          <span className="text-xs uppercase tracking-widest font-semibold text-amber-300 bg-white/10 px-3 py-1 rounded-full border border-white/20 inline-block mb-4">Our Journey</span>
          <h2 className="text-3xl sm:text-4xl font-display font-bold text-white tracking-tight">Seven stages of the KKN-PLP journey</h2>
          <p className="mt-4 text-slate-300">From arrival to a heartfelt closing — every stage of presence, learning, and service.</p>
        </div>
        {loading ? (
          <div className="space-y-6">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 rounded-xl bg-white/5 animate-pulse" />)}</div>
        ) : sorted.length === 0 ? (
          <EmptyState title="No Timeline Yet" message="The journey timeline will appear here once added." testId="timeline-empty" />
        ) : (
