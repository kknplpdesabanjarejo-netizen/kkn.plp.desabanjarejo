  ...
  return (
    <section id="location" className="py-24 bg-slate-50 dark:bg-slate-900" data-testid="location-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <SectionHeader label="Lokasi" title="Lokasi Pengabdian" description="Kenali lokasi tempat kami belajar, berkegiatan, dan mengabdi bersama masyarakat." testId="location-header" />
          ...
          <div className="lg:col-span-2 rounded-3xl border border-slate-100 bg-white dark:bg-slate-800 dark:border-white/10 p-8 flex flex-col">
            ...
            {loc.googleMapsUrl && (
              <a href={loc.googleMapsUrl} target="_blank" rel="noreferrer" data-testid="location-directions" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-emerald-800 hover:text-emerald-600">
                <Navigation className="h-4 w-4" /> Lihat di Google Maps
              </a>
            )}
          </div>
            ...
            {loc.embedUrl ? (
              ...
              <div className="w-full h-full min-h-[320px] grid place-items-center text-center p-8 grid-canvas">
                <div>
                  ...
                  <p className="mt-3 font-display font-semibold text-slate-700 dark:text-slate-200">Peta segera hadir</p>
                  <p className="text-sm text-slate-400 mt-1">Lokasi belum tersedia.</p>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <section id="contact" className="py-24 bg-emerald-950 relative overflow-hidden" data-testid="contact-section">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative">
        <span className="text-xs uppercase tracking-widest font-semibold text-amber-300 bg-white/10 px-3 py-1 rounded-full border border-white/20 inline-block mb-4">Kontak</span>
        <h2 className="text-3xl sm:text-4xl font-display font-bold text-white tracking-tight">Hubungi Kami</h2>
        <p className="mt-4 text-emerald-50/80 max-w-xl mx-auto">Untuk informasi lebih lanjut mengenai kegiatan KKN-PLP Terpadu Kelompok 66, silakan hubungi kami melalui kanal komunikasi yang tersedia.</p>
          ...
          {socials.map(([label, Icon, href]) =>
            href ? (
              ...
              <span key={label} className="flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-5 py-3 text-white/40 font-medium cursor-not-allowed" data-testid={`contact-${label.toLowerCase()}-soon`}>
                <Icon className="h-5 w-5" /> {label} · Segera Hadir
              </span>
            )
          )}
        </div>
