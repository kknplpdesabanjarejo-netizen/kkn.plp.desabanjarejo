...
export function Location({ location }) {
  ...
  return (
    <section id="location" className="py-24 bg-slate-50" data-testid="location-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid lg:grid-cols-5 gap-8 items-stretch">
          <div className="lg:col-span-2 rounded-3xl border border-slate-100 bg-white p-8 flex flex-col">
            ...
            <h3 className="font-display font-bold text-xl text-slate-900">{loc.village || "[VILLAGE NAME]"}</h3>
            <p className="mt-2 text-slate-600">{place || "[SUBDISTRICT], [REGENCY], [PROVINCE]"}</p>
            {loc.address && <p className="mt-3 text-sm text-slate-500">{loc.address}</p>}
            {loc.googleMapsUrl && (
          ...
          <div className="lg:col-span-3 rounded-3xl overflow-hidden border border-slate-100 bg-white min-h-[320px]">
            {loc.embedUrl ? (
